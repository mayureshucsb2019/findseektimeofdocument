...
int main(){
  ...
  for(int loop=1; loop<1001; loop++){
    for(int file_no = 1; file_no < 151; file_no++ ){
      ...
      std::cout << std::chrono::duration_cast<std::chrono::nanoseconds>(finish-start).count() <<"ns "<<"Average time: "<<time_total/file_no<<"............."<<loop<<"......"<<file_name_value+document_name_value<<"Time 150 average: "<<time_total150/(loop+1)<<endl;
