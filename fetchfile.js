...
mongoose
    ...
    .then(result => {
        ...
        server.listen(process.env.APPLICATION_PORT, async function() {
            ...
            // read the json file
            hashIds = await JSON.parse(fs.readFileSync("./hashIds.json"));
            // get the file list from the json file
            hashIds = hashIds.hashes;
            ...
            // console.log("random indices is ", indiceList)
            // console.log("object ids are :",objectIdOfFiles[indiceList[0]], objectIdOfFiles[indiceList[1]])
            // let res = await Promise.all([
            //     fetchfiles(objectIdOfFiles[indiceList[0]]), fetchfiles(objectIdOfFiles[indiceList[1]]) 
            // ]).catch((err) => {
            //         console.log("error at ", err.message);
            //     });
            // console.log(res);
            //console.log(hashIds);
            //console.log(objectIdOfFiles);
...
// fetchfiles(objectIdOfFiles[indiceList[491]]), fetchfiles(objectIdOfFiles[indiceList[492]]), fetchfiles(objectIdOfFiles[indiceList[493]]), fetchfiles(objectIdOfFiles[indiceList[494]]), fetchfiles(objectIdOfFiles[indiceList[495]]), fetchfiles(objectIdOfFiles[indiceList[496]]), fetchfiles(objectIdOfFiles[indiceList[497]]), fetchfiles(objectIdOfFiles[indiceList[498]]), fetchfiles(objectIdOfFiles[indiceList[499]]), 
                ...
                // let fileid = objectIdOfFiles[Math.floor(Math.random() * Math.floor(len))];
                // let x= await fetchfiles(fileid);
                totalTime += Date.now() - starttime ;
                console.log("Iterataion ", i," : Files fetched in time: ",Date.now() - starttime," total time: ",totalTime, " average time: ",totalTime/(i+1));
